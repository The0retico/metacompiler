...
class Grammar implements
		Iterable<Entry<String, Entry<ImmutableList<String>, String>>> {
	...
	private final ImmutableListMultimap<String, Entry<ImmutableList<String>, String>> rules;

	public Grammar(
			final ImmutableListMultimap<String, Entry<ImmutableList<String>, String>> grammarRules) {
	...
	private ImmutableSet<Character> first(final String symbol) {
		...
		if (IS_TERMINAL.apply(symbol)) {
			...
			for (final Entry<ImmutableList<String>, String> production : rules
					.get(symbol)) {
				if (production.getKey().size() == 1
						&& production.getKey().get(0) == String
								.valueOf(EPSILON)) {
					result.add(EPSILON);
				} else {
					Preconditions.checkNotNull(production);
					Preconditions.checkArgument(production.getKey().size() > 0);
	...
	public ImmutableSet<Character> follow(final String nonTerminal) {
		...
		final Predicate<Entry<String, Entry<ImmutableList<String>, String>>> withNonTerminal = new Predicate<Entry<String, Entry<ImmutableList<String>, String>>>() {

			@Override
			public boolean apply(
					final Entry<String, Entry<ImmutableList<String>, String>> entry) {
				return entry.getValue().getKey().contains(nonTerminal);
			}
		};
		final Iterable<Entry<String, Entry<ImmutableList<String>, String>>> productionsWithNonTerminal = Iterables
				.filter(rules.entries(), withNonTerminal);
		for (final Entry<String, Entry<ImmutableList<String>, String>> production : productionsWithNonTerminal) {
			final ImmutableList<String> value = production.getValue().getKey();
	...
	@Override
	public Iterator<Entry<String, Entry<ImmutableList<String>, String>>> iterator() {
