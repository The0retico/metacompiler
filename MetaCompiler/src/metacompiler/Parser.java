...
class Parser {
	...
	public static Table<Character, String, ImmutableList<String>> createTable(
			final Grammar grammar) {
		Preconditions.checkNotNull(grammar);
		LOGGER.info("start");
		final Table<Character, String, ImmutableList<String>> result = HashBasedTable
				.create();
		for (final Entry<String, ImmutableList<String>> rule : grammar) {
			...
			if (grammar.first(rule.getValue()).contains(Grammar.EPSILON)) {
	...
	private final Table<Character, String, ImmutableList<String>> parsingTable;
	...
	public ImmutableList<Entry<String, ImmutableList<String>>> analyze(
			final Reader reader) throws IOException, LexerException,
			ParserException {
		Preconditions.checkNotNull(reader);
		final ImmutableList.Builder<Entry<String, ImmutableList<String>>> parseTree = ImmutableList
				.builder();
	...
	private Entry<String, ImmutableList<String>> reduce(
		...
		final ImmutableList<String> production = parsingTable.get(lookahead,
				nonTerminal);
		if (production == null) {
			throwParsingException(nonTerminal, input, lookahead);
		}
		for (int index = production.size() - 1; index >= 0; index--) {
			final String symbol = production.get(index);
